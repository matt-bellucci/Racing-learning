	...
	def __build_model__(self):
		...
		if len(self.structure) > 2:
			for i, k in enumerate(self.structure[2:]):
		 		...
		 		model.add(Dense(k, weights=chromo_to_weights(self.chromos[i+1])))
...
class Population:
	...
	def __init__(self, n_individus, neural_structure, initial_chromosomes, fit_function, individuals=None):
		if not individuals or len(individuals) != n_individus:
			self.individuals = [Individu(copy.deepcopy(initial_chromosomes), neural_structure, fit_function) for x in range(n_individus)]
		else:
	...
	def best_score(self):
		rank = self.rank_fitness()
		return self.individuals[rank[0]].fitness
...
class Genetic:
	...
	def train(self, n_bests=2, weights=None, mutation_chance=0.25, n_proc=1):
		if n_bests < 2:
			n_bests = 2
